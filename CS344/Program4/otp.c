...
// For printing errors
void error(const char *msg, int value) { perror(msg); exit(value); }

/**
Grabs and parses files for enc/dec functions
Inputs:
	plaintext	The name of the input text file
	key			The name of the key text file
	port_arg	The port number as a string
	enc_dec		Decides functionality between enc & dec
**/
...
{	
	...
	// Printing and terminating program
	printf("%s\n", output);
	close(socketFD);
	return 0;
}
